...
@section('content')
    <div class="container mx-auto my-6 p-6 bg-white shadow-lg rounded-lg">
        <h1 class="text-2xl font-bold text-gray-700 mb-4">Programar Sesión</h1>
        <form action="{{ route('sessions.store') }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <select class="form-control block w-full mt-1 p-2 border border-gray-300 rounded-lg" id="psicologa_id" name="psicologa_id" required>
                    @foreach($psicologas as $psicologa)
                        <option value="{{ $psicologa->id }}">{{ $psicologa->nombre_usuario }}</option>
                    @endforeach
                </select>
            </div>
            <div class="mb-4">
            ...
            <div class="mb-4">
                ...
                <input type="text" class="block w-full mt-1 p-2 border border-gray-300 rounded-lg" id="tipo_sesion" name="tipo_sesion" required>
            </div>
            <div class="mb-4">
                <label for="descripcion" class="block text-gray-700">Descripción</label>
                <textarea class="block w-full mt-1 p-2 border border-gray-300 rounded-lg" id="descripcion" name="descripcion"></textarea>
            </div>
            <div class="mb-4">
                <label for="fecha_hora" class="block text-gray-700">Fecha y Hora</label>
                <input type="datetime-local" class="block w-full mt-1 p-2 border border-gray-300 rounded-lg" id="fecha_hora" name="fecha_hora" required>
            </div>
            <button type="submit" class="btn btn-primary inline-block px-6 py-2 text-white bg-indigo-600 hover:bg-indigo-700 rounded-lg">Programar Sesión</button>
        </form>
    </div>
@endsection
