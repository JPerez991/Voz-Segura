            ...
            <div class="flex">
                <div class="shrink-0 flex items-center space-x-2">
                    <a href="{{ route('dashboard') }}" class="flex items-center">
                        <x-logo-heart class="block h-9 w-auto fill-current text-purple-800" />
                ...
                <x-dropdown align="right" width="48">
                    <x-slot name="trigger">
                        <button class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-purple-600 bg-white hover:text-purple-700 focus:outline-none transition ease-in-out duration-150">
                            <div id="nav-username">{{ Auth::check() ? Auth::user()->nombre_usuario : 'Invitado' }}</div>
                            <div class="ms-1">
                    ...
                    <x-slot name="content">
                        ...
                        <form method="POST" action="{{ route('logout') }}">
                            ...
                            <x-dropdown-link :href="route('logout')"
                                onclick="event.preventDefault(); this.closest('form').submit();" class="text-purple-800">
                                {{ __('Log Out') }}
                            </x-dropdown-link>
                        </form>
                        @else
